...
- `Loot Timeout` - Time in seconds before a looting bot is timed out and stops looting
- `Log Levels` - Enable different levels of log messages to show in the logs
...
- `Maximum concurrent scans` - Max number of bots that can scan for loot at the same time. Takes effect next raid. A value of 0 represents no limit
- `Maximum unsuccessful attempts` - Max number of unsuccessful attempts a bot can try to find loot before taking a break. A value of 0 represents no limit
- `Empty attempts cooldown` - How long (in seconds) to wait before trying to find loot when max attempts is reached.
...
- [x] Stronger checks for when to equip a new primary weapon
...
- `user/mods/Skwizzy.LootingBots` - Provide the option to clear out the loot that PMC/Scav bots start with in their backpacks. This does not include meds, ammo, grenades ect. These options can be found in the `Skwizzy.LootingBots/config/config.json`.
